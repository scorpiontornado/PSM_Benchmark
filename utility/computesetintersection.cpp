...
void ComputeSetIntersection::ComputeCNMergeBasedAVX2(const VertexID* larray, const ui l_count,
                                                     ...
                                                     VertexID* cn, ui &cn_count) {
    cn_count = 0;
    // LOG() << "larray address = " << larray << ", l_count = " << l_count << std::endl;
        ...
        if (li + 1 < lc && ri + 3 < rc) {
            ...
            while (true) {
                ...
                if (larray[li + 1] == rarray[ri + 3]) {
                    ...
                    // if (li >= l_count) {
                    //     std::cout << "out of range, error of intersection!" << std::endl;
                    // }
                    u_elements = _mm256_loadu_si256((__m256i *) (larray + li));
                    u_elements_per = _mm256_permutevar8x32_epi32(u_elements, per_u_order);
                }
...
void ComputeSetUnion::ComputeCandidates(const VertexID* larray, const ui l_count,
                                               const VertexID* rarray, const ui r_count,
                                               VertexID* cn, ui &cn_count) {
#if HYBRID == 0
    #if SI == 0
    if (l_count / 50 > r_count || r_count / 50 > l_count) {
        return ComputeUnionGallopingAVX2(larray, l_count, rarray, r_count, cn, cn_count);
    }
    else {
        return ComputeUnionGallopingAVX2(larray, l_count, rarray, r_count, cn, cn_count);
    }
...
void ComputeSetUnion::ComputeUnionGallopingAVX2(const VertexID* larray, const ui l_count,
                            const VertexID* rarray, const ui r_count,
                            VertexID* cn, ui& cn_count) {
  cn_count = 0;
  ...
  if (li < lc) {
    std::copy(larray + li, larray + lc, cn + cn_count);
    cn_count += lc - li;
  }
  if (ri < rc) {
    std::copy(rarray + ri, rarray + rc, cn + cn_count);
    cn_count += rc - ri;
  }
}
