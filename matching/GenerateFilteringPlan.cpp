...
void GenerateFilteringPlan::generateCFLFilterPlan(const Graph *data_graph, UnitArgs* u_args) {
    VertexID* tree_order = u_args->tree_order;
    ...
    GraphOperations::bfsTraversal(query_graph, start_vertex, tree, tree_order);

    std::vector<ui> order_index(query_vertices_num);
    for (ui i = 0; i < query_vertices_num; ++i) {
        VertexID query_vertex = tree_order[i];
        order_index[query_vertex] = i;
    }

    ui level_count = -1;
    ui* level_offset = new ui[query_vertices_num + 1];

    for (ui i = 0; i < query_vertices_num; ++i) {
        VertexID u = tree_order[i];
...
/*
    这里是把bn_和fn_给弄好
 */
void GenerateFilteringPlan::generateDPisoFilterPlan(const Graph *data_graph, UnitArgs* u_args) {
    // LOG() << "vertices_count : " << u_args->query_graph->subgraph->getVerticesCount() << std::endl;
    VertexID* tree_order = u_args->tree_order;
    TreeNode* tree = u_args->tree;
    Graph* query_graph = u_args->query_graph->subgraph;
    VertexID start_vertex = selectDPisoStartVertex(data_graph, query_graph);
    // LOG() << "st_vertex : " << start_vertex << std::endl;
    GraphOperations::bfsTraversal(query_graph, start_vertex, tree, tree_order);
...
void GenerateFilteringPlan::generateCECIFilterPlan(const Graph *data_graph, UnitArgs* u_args) {
    // LOG() << "vertices_count : " << u_args->query_graph->subgraph->getVerticesCount() << std::endl;
    VertexID* tree_order = u_args->tree_order;
    TreeNode* tree = u_args->tree;
    Graph* query_graph = u_args->query_graph->subgraph;    
...
// 这里先用NLF计算一遍candi数量, count / degree 就是得分，最小得分的作为 start_vertex 
