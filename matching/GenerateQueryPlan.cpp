...
void GenerateQueryPlan::generateGQLQueryPlan(const Graph *data_graph, const Graph *query_graph, 
                                             UnitArgs *u_args) {
    ...
    /*
        给GQL加一个order BFS Tree，为了计算workload做准备
    */
    TreeNode* new_tree = new TreeNode[query_graph->getVerticesCount()];
    ui* new_tree_order = new ui[query_graph->getVerticesCount()];
    
    auto start = std::chrono::steady_clock::now();

    GraphOperations::bfsTraversalOrdered(query_graph, order[0], new_tree, new_tree_order, order);

    auto end = std::chrono::steady_clock::now();
    int64_t enumeration_time_in_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
    LOG() << "bfsTraversalOrdered time == " << NANOSECTOSEC(enumeration_time_in_ns) << std::endl;

    start = std::chrono::steady_clock::now();
    // computeWorkloadArray(u_args, query_vertices_num, new_tree_order, new_tree, data_graph, candidates_count, candidates, workload_array);
    computeWorkloadArray(u_args, query_vertices_num, new_tree_order, new_tree, edge_matrix, candidates_count, candidates, workload_array);

    end = std::chrono::steady_clock::now();
    enumeration_time_in_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
    LOG() << "computeWorkloadArray time == " << NANOSECTOSEC(enumeration_time_in_ns) << std::endl;
}
...
void GenerateQueryPlan::generateCFLQueryPlan(const Graph *query_graph, UnitArgs* u_args) {
    Edges*** edge_matrix = u_args->edge_matrix;
    ui *candidates_count = u_args->candidates_count;
    ui query_vertices_num = query_graph->getVerticesCount();
    ...
    VertexID root_vertex = tree_order[0];
    // order = new ui[query_vertices_num];
    // pivot = new ui[query_vertices_num];
    std::vector<bool> visited_vertices(query_vertices_num, false);
...
/*
    这段代码定义了一个函数 checkQueryPlanCorrectness，用于检查查询计划（order 和 pivot 数组）的正确性。查询计划的正确性包括：

    每个查询图中的顶点是否都包含在查询顺序 order 中。
    查询顺序是否是连通的（通过检查 pivot 中的关系）。
 */
...
void GenerateQueryPlan::checkQueryPlanCorrectness(const Graph *query_graph, ui *order, ui *pivot) {
    ...
    for (ui i = 1; i < query_vertices_num; ++i) {
        ...
        // LOG() << "vertex == " << vertex << ", pivot_vertex == " << pivot_vertex << std::endl;
...
void GenerateQueryPlan::generatePivot(UnitArgs* u_args) {
    ui* pivot = u_args->pivots;
    TreeNode* tree = u_args->tree;
    ui* order = u_args->order;
    for (ui i = 1; i < u_args->vertices_count; ++i) {
...
// 单独的函数：计算权重数组
void
GenerateQueryPlan::computeWeightArray(ui query_vertices_num,
                        const ui* tree_order,
                        TreeNode* tree,
                        Edges*** edge_matrix,
                        const ui* candidates_count,
                        ui** weight_array) {
    // 从后向前遍历所有查询顶点
    for (int i = query_vertices_num - 1; i >= 0; --i) {
        VertexID vertex = tree_order[i];
        ...
        for (ui j = 0; j < node.fn_count_; ++j) {
            ...
            // 如果孩子节点满足条件（bn_count_为1），则更新权重数组
            if (child_node.bn_count_ == 1) {
                ...
                for (ui k = 0; k < candidates_count[vertex]; ++k) {
                    ...
                    for (ui l = 0; l < cur_candidates_count; ++l) {
                        ui candidate = cur_candidates[l];
                        weight += weight_array[child][candidate];
                    }
        ...
        // 如果没有孩子节点满足条件，则将该顶点所有候选位置的权重设置为1
        if (set_to_one) {
            std::fill(weight_array[vertex],
                      weight_array[vertex] + candidates_count[vertex],
                      1);
        }
    }
}
...
void
GenerateQueryPlan::computeTotalWorkload(UnitArgs* u_args) {
    ui* order = u_args->order;
    VertexID u = order[0];
    auto& workload_array = u_args->workload_array;
    auto& u_workload_array = workload_array[u];
    int64_t total_workload = 0;
    for (auto& [v, v_workload] : u_workload_array) {
        total_workload += v_workload;
    }
    u_args->total_workload = total_workload;
}

void
GenerateQueryPlan::initWorkload(ui query_vertices_num, const ui* tree_order, const ui* candidates_count, ui** candidates,
                std::vector<std::unordered_map<VertexID, int64_t>>& workload_array)
{
    // TODO: 所有 workload_array 统一，直接map也是可以的，但是得统一，不然后面没法用
    // 根据 TE_Candidates ，把所有节点的 workload_array 初始化为1
    workload_array.resize(query_vertices_num);
    for (int i = 0; i < query_vertices_num; ++i) {
        VertexID vertex = tree_order[i];
        
        workload_array[vertex] = std::unordered_map<VertexID, int64_t>();
        for (ui j = 0; j < candidates_count[vertex]; ++j) {
            VertexID candidate = candidates[vertex][j];
            workload_array[vertex][candidate] = 1;
        }
    }
}
...
// 单独的函数：计算权重数组
void
GenerateQueryPlan::computeWorkloadArray(UnitArgs *u_args, ui query_vertices_num, const ui* tree_order, TreeNode* tree,
                          Edges*** edge_matrix, const ui* candidates_count, ui** candidates,
                          std::vector<std::unordered_map<VertexID, int64_t>>& workload_array)
{
    initWorkload(query_vertices_num, tree_order, candidates_count, candidates, workload_array);
    for (int i = query_vertices_num - 1; i >= 0; --i) {
        VertexID vertex = tree_order[i];
        // LOG() << "current vertex == " << vertex << std::endl;
        TreeNode& node = tree[vertex];


        // 遍历当前节点的所有孩子节点

        for (ui j = 0; j < node.children_count_; ++j) {
            VertexID child = node.children_[j];
            ...
            Edges& cur_edge = *edge_matrix[vertex][child];
            for (ui k = 0; k < candidates_count[vertex]; ++k) {
                VertexID vertex_candidate = candidates[vertex][k];
                ui cur_candidates_count = cur_edge.offset_[k + 1] - cur_edge.offset_[k];
                ui* cur_candidates = cur_edge.edge_ + cur_edge.offset_[k];

                ui workload = 0;
                // LOG() << "cur_candidates_count == " << cur_candidates_count << std::endl;
                for (ui l = 0; l < cur_candidates_count; ++l) {
                    ui candidate_idx = cur_candidates[l];
                    VertexID child_candidate = candidates[child][candidate_idx];
                    workload += workload_array[child][child_candidate];
                    // LOG() << "candidate_idx == " << candidate_idx << ", child_candidate == " << child_candidate << std::endl;
                    // LOG() << "workload_array[child][child_candidate] == " << workload_array[child][child_candidate] << std::endl;
                }

                // workload 是 child 之和
                workload_array[vertex][vertex_candidate] *= workload;
                // LOG() << "vertex == " << vertex << ", vertex_candidate == " << vertex_candidate << ", workload == " << workload << std::endl;
            }
        }

        // // 如果没有孩子节点满足条件，则将该顶点所有候选位置的权重设置为1
        // if (set_to_one) {
        //     std::fill(workload_array[vertex], workload_array[vertex] + candidates_count[vertex], 1);
        // }
    }

    VertexID root = u_args->order[0];
    double_t total_workload = 0;
    for (ui k = 0; k < candidates_count[root]; ++k) {
        VertexID vertex_candidate = candidates[root][k];
        total_workload += workload_array[root][vertex_candidate];
    }
    u_args->total_workload = total_workload;
}

// 这里要拿idx，但是TE_Candidates的逻辑下是拿不到idx的
void
GenerateQueryPlan::computeWorkloadArray(UnitArgs *u_args, ui query_vertices_num, const ui* tree_order, TreeNode* tree,
        ...
        std::vector<std::unordered_map<VertexID, int64_t>>& workload_array)
{
    ...
    for (int i = query_vertices_num - 1; i >= 0; --i) {
        ...
        // 找到当前节点的parent节点，遍历他的candidates
        VertexID u_p = tree[vertex].parent_;
        for (ui j = 0; j < candidates_count[u_p]; ++j) {
            VertexID v_p = candidates[u_p][j];
            
            auto iter = TE_Candidates[vertex].find(v_p);
            if (iter == TE_Candidates[vertex].end() || iter->second.empty()) {
                break;
            }

            int64_t workload = 0;
            std::vector<VertexID>& v_p_nbrs = iter->second;
            for (VertexID& v_p_nbr : v_p_nbrs) {
                workload += workload_array[vertex][v_p_nbr];
            }

            workload_array[u_p][v_p] *= workload;
        }

        // // 如果没有孩子节点满足条件，则将该顶点所有候选位置的权重设置为1
        // if (set_to_one) {
        //     for (auto& [v, w] : workload_array[vertex]) {
        ...
        std::vector<std::unordered_map<VertexID, int64_t>>& workload_array)
{
    ...
    //             ui child_candidate_count = candidates_count[child];
    //             ui workload = 0;
    //             for (ui l = 0; l < child_candidate_count; ++l) {
    //                 VertexID v_child = candidates[child][l];

    //                 if (data_graph->checkEdgeExistence(vertex_candidate, v_child)) {
    //                     workload += workload_array[child][v_child];
    //                 }  
    //             }
...
void
GenerateQueryPlan::printWorkloadArray(Graph* q_graph, UnitArgs* u_args) {
    GraphOperations::printBFSTree(u_args->tree, u_args->vertices_count);

    ui query_vertices_num = u_args->query_graph->getVerticesCount();
    ui *candidates_count = u_args->candidates_count;
    std::vector<std::unordered_map<VertexID, int64_t>>& workload_array = u_args->workload_array;
    
    std::ostringstream oss;
    oss << "----------printWorkloadArray----------" << std::endl;
    for (ui i = 0; i < query_vertices_num; ++i) {
        oss << "vertex = " << i << std::endl;
        for (ui j = 0; j < candidates_count[i]; ++j) {
            VertexID candidate = u_args->candidates[i][j];
            oss << workload_array[i][candidate] << " ";
        }
        oss << std::endl;
    }
    LOG() << oss.str();
...
/*
    总之这个函数就是自底向上 weight_array 数组
    weight_array 和 candidates数组是对应的
*/
void GenerateQueryPlan::generateDPisoQueryPlan(const Graph *query_graph, UnitArgs* u_args) {
    Edges*** edge_matrix = u_args->edge_matrix;
    ui *candidates_count = u_args->candidates_count;
    ui query_vertices_num = query_graph->getVerticesCount();
    ui* tree_order = u_args->tree_order;
    ...
    //     // LOG() << "fn_count_ : " << node.fn_count_ << std::endl;
    //     for (ui j = 0; j < node.fn_count_; ++j) {
    //         VertexID child = node.fn_[j];
    //         TreeNode& child_node = tree[child];

    //         // LOG() << "child_node.bn_count_ : " << child_node.bn_count_ << std::endl;
    //         if (child_node.bn_count_ == 1) {
    //             set_to_one = false;
    //             Edges& cur_edge = *edge_matrix[vertex][child];
    //             for (ui k = 0; k < candidates_count[vertex]; ++k) {
    ...
    //                     // LOG() << "child = " << child << ", candidates = " << candidates << std::endl;
    ...
    // 调用独立函数计算权重数组
    computeWeightArray(query_vertices_num, tree_order, tree, edge_matrix, candidates_count, weight_array);
    computeWorkloadArray(u_args, query_vertices_num, tree_order, tree, edge_matrix, candidates_count, candidates, workload_array);
}

void GenerateQueryPlan::generateCECIQueryPlan(const Graph *query_graph, UnitArgs* u_args) {
    ui query_vertices_num = query_graph->getVerticesCount();
    // LOG() << "query_vertices_num : " << query_vertices_num << std::endl;
    ui* tree_order = u_args->tree_order;
    TreeNode* tree = u_args->tree;
    ...
    ui* candidates_count = u_args->candidates_count;
    ui** candidates = u_args->candidates;
    std::vector<std::unordered_map<VertexID, std::vector<VertexID >>>& TE_Candidates = u_args->TE_Candidates;
    auto& workload_array = u_args->workload_array;
    ...
    computeWorkloadArray(u_args, query_vertices_num, tree_order, tree, TE_Candidates, candidates_count, candidates, workload_array);
}
